...
if (isset($_SESSION['user']) && unserialize($_SESSION['user']) instanceof \app\models\Medecin) {
    ...
    if (isset($_GET['page']) && $_GET['page'] == "consultation") {
        ...
        die();
    } elseif (isset($_GET['page']) && $_GET['page'] == 'imprimerOrdonnance' && isset($_GET['idOrdonnance'])) {
        $ordonnance = \app\Config::getInstance()->getEntityManager()->find(\app\R::ORDONNANCE, $_GET['idOrdonnance']);
        /**
         * @var $ordonnance \app\models\Ordonnance
         */
        $fpdf = new \app\ImpressionOrdonnance($ordonnance->getConsultation()->getPatient());
        $fpdf->loadOrdonnance($ordonnance);
        $fpdf->Output();
